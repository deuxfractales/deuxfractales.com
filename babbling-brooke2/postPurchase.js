...
async function postPurchase (fastify, options) {
    ...
    fastify.post('/postPurchase', async (request, reply) => {

        let purchases = request.body.purchasedItems
        purchases.forEach(function (purchase){

            let beatName = purchase.title
            const attachmentMp3 = fs.readFileSync(`./storage/mp3/${beatName}.mp3`).toString("base64");
            const attachmentWav = fs.readFileSync(`./storage/wav/${beatName}.wav`).toString("base64");
            const attachmentZip = fs.readFileSync(`./storage/zip/${beatName}.zip`).toString("base64");
